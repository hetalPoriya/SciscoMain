...
class DealerKYC extends StatefulWidget {
  @override
  _DealerKYCState createState() => _DealerKYCState();
}

class _DealerKYCState extends State<DealerKYC> {
  final _gstinCtrl = TextEditingController();
  final _panNoCtrl = TextEditingController();
  final _drugLicenseNoCtrl = TextEditingController();
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            RoundedButton(
              ...
              press: () {
                if (_gstinCtrl.text.toString().isEmpty) {
                  _showMyDialog(context, "Please Enter GSTIN Number");
                } else if (_panNoCtrl.text.toString().isEmpty) {
                  _showMyDialog(context, "Please Enter Pan Number");
                } else if (_drugLicenseNoCtrl.text.toString().isEmpty) {
                  _showMyDialog(context, "Please Enter Drug License Number");
                } else if (_aadharFile == null) {
                  _showMyDialog(context, "Please Select your Aadhar");
                } else {
                  submitKYCReq();
                }
              },
  ...
  Future<Response> submitKYCReq() async {
    ...
    try {
      ...
      Map<String, dynamic> formData = {
        ...
        "gstin": _gstinCtrl.text.toString(),
        "drug_license_no": _drugLicenseNoCtrl.text.toString(),
      };
  ...
  Future<void> captureImage(ImageSource imageSource) async {
    try {
      ...
      setState(() {
        ...
        fileitems.add(_aadharFile);
        // fileitemsStr.add(Utility.base64String(_aadharFile.readAsBytesSync()));
        fileitemsStr.add(_aadharFile.path);
