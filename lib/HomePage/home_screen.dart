...
class MyHomePage extends StatefulWidget {
  const MyHomePage({Key key, this.id, this.superCategoryName, this.pageName})
      : super(key: key);
  final String id;
  ...
  _MyHomePageState createState() =>
      _MyHomePageState(this.id, this.superCategoryName, this.pageName);
}
...
class _MyHomePageState extends State<MyHomePage> with TickerProviderStateMixin {
                    ...
                    child: productsList.length > 0
                        ? ListView.separated(
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              return ItemGrid(
                                productsListDatum: productsList[index],
                                callBack: () {
                                  setState(() {
                                    // for (int i = 0 ; i< selectedProducts.length ; i++) {
                                    if (indexList[index].isSelected) {
                                      indexList[index].isSelected = false;
                ...
                color: AppTheme.appColor,
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text('${selectedProducts.length.toString()} ${selectedProducts.length>1?'Items':'Item'}',style: TextStyle(
                          fontSize: 15,
  ...
  onItemunSelected(int index) {
        ...
        if (selectedProducts != null && selectedProducts.isNotEmpty) {
          for (ProductsList productList in selectedProducts) {
            if (productsList[i].id == productList.id) {
              ...
              prefs.setString(SharedPrefKey.SELECTEDPRODUCTS,
    ...
    print(index.toString() +
        " POSITION-------UNSELECTED"); /*json.encode(mBookingSubmitModel)*/
  }
  ...
  Future<Response> getBrandsList() async {
    ...
    if (isDeviceOnline) {
      ...
      try {
        ...
        Response response = await baseApi.dio.post(brandByIDUrl, data: body);
  ...
  Future<Response> getProductsList() async {
    ...
    if (isDeviceOnline) {
      ...
      try {
        BaseApi baseApi = new BaseApi();
        print(prefs.getString(SharedPrefKey.USERID));
        var body = json.encode({
          "Categoryid": id,
          "user_id": prefs.getString(SharedPrefKey.USERID),
        });
        Response response = await baseApi.dio.post(productByCatUrl, data: body);
        progressDialog.dismiss();
  ...
  Future<Response> getaQuote() async {
    bool isDeviceOnline = true;
    SharedPreferences prefs = await SharedPreferences.getInstance();
    // bool isDeviceOnline = await checkConnection();
    if (isDeviceOnline) {
...
//                    gridItem.title,
                  ...
                  widget.cardselected
                      ...
                      : Align(
                          ...
                          child: Padding(
                              padding:
                                  EdgeInsets.only(right: 10, top: 0, left: 10),
                              child: ButtonTheme(
                                  buttonColor: !widget.cardselected
                                      ? Colors.white
                                      : AppTheme.appColor,
                                  minWidth: 70.0,
                                  height: 30.0,
